...
public class GPSService extends Service implements LocationListener {
	...
	public void onCreate() {
		...
		firstFixListeners = new ArrayList<GPSService.FirstFixListener>();
		locationUpdateListeners = new ArrayList<GPSService.LocationUpdateListener>();
		startUsingGPS(new FirstFixListener() {

			@Override
			public void onFirsFixChanged(boolean hasGPSfix) {
				Bundle bundle = new Bundle();
				bundle.putBoolean(GPSResultReceiver.EXTRA_HASFIRSTFIX,
						hasGPSfix);
				mReceiver.send(GPSResultReceiver.RESULTCODE_FIRST_FIX_CHANGE,
						bundle);
			}
			...
			public void onLocationChanged(Location location) {
				Bundle bundle = new Bundle();
				bundle.putParcelable(GPSResultReceiver.EXTRA_LOCATION, location);
				mReceiver.send(GPSResultReceiver.RESULTCODE_LOCATION_UPDATE,
						bundle);
	...
	private Location startLocationListener() {
		...
		try {
			...
			if (isGPSEnabled) {
				...
				Bundle bundle = new Bundle();
				bundle.putString(GPSResultReceiver.EXTRA_ERRORMSG,
						"GPS not available, make sure it is enabled");
				mReceiver.send(GPSResultReceiver.RESULTCODE_ERROR, bundle);
			}
